...
def _alloc_lines(alloc):
    if not alloc:
        return ["💵 CASH — 100% Cash (nicht investiert)"]
    lines = ["📈 INVESTIERT:"]
    tot = 0.0
    for a, w in sorted(alloc.items(), key=lambda kv: -kv[1]):
        m = ASSETS[a]; tot += w
        lev = f"{m['leverage']}x" if m["leverage"] != 1 else "1x"
        lines.append(f"   • {int(round(w*100))}% {m['display']} ({lev}) — {m['product']}")
    cash = 1.0 - tot
    if cash > 0.005:                                   # teil-investiert -> Cash-Rest ausweisen
        lines.append(f"   • {int(round(cash*100))}% Cash (Fallback)")
    return lines


def _asset_detail_lines(alloc, ind, rebound, i):
    """Pro-Baustein-Detail (Reverse: SMA-Rang + Rebound; GTAA: Momentum + Trend)."""
    out = []; sel = set(alloc.keys())
    if CFG["mode"] == "reverse":
        rankv = ind["rank_val"]
        valid = [a for a in ASSET_LIST if pd.notna(rankv[a].iloc[i])]
        order = sorted(valid, key=lambda a: rankv[a].iloc[i])
        flop_set = set(order[:CFG["N"]])
        for a in order:
            rv = rankv[a].iloc[i]; rb = rebound[a].iloc[i]
            mark = "✅ gehalten" if a in sel else ("· Flop-2, aber Rebound≤0 → Cash" if a in flop_set
                                                   else "· kein Flop-2 (zu hoher SMA-Rang)")
            out.append(f"   • {ASSETS[a]['display']}: SMA{CFG['S']}/SMA{CFG['L']} {_fmt_pct(rv)}, "
                       f"Rebound(1+2+3M) {_fmt_pct(rb)}  {mark}")
    else:
        mom = ind["mom"]; trend = ind["trend"]
        order = sorted(ASSET_LIST, key=lambda a: mom[a].iloc[i] if pd.notna(mom[a].iloc[i]) else -1e9, reverse=True)
        for a in order:
            mv = mom[a].iloc[i]; tr = bool(trend[a].iloc[i]); elig = tr and (mv == mv) and mv > 0
            mark = "✅ gehalten" if a in sel else ("· qualifiziert" if elig else "· raus (Trend/Mom)")
            out.append(f"   • {ASSETS[a]['display']}: Momentum {_fmt_pct(mv)}, "
                       f"Trend SMA{CFG['S']}>SMA{CFG['L']} {'JA' if tr else 'nein'}  {mark}")
    return out
...
def _signal_snapshot(ind, rebound, i):
    ...
    for a in ASSET_LIST:
        ...
        if CFG["mode"] == "reverse":
            rec["rankValue"] = _num(ind["rank_val"][a].iloc[i])
